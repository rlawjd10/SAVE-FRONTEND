...
class ChildRecordRVAdapter (private val recordList : ArrayList<RecordData>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if (position == recordList.size) {
            ...
            (holder as ViewHolderRecord).bind(recordList[position])
            (holder as ViewHolderRecord).itemView.setOnClickListener{  mItemClickListener.onItemClick(recordList[position])}
        }
    }
